...
train_feats = pd.read_hdf('data/training_feats/training_set_feats_std.h5', mode='r')
...
for i, (_train, _eval) in enumerate(folds.split(y_tgt, y_tgt)):
    ...
    val_loss_str = '{:.4}'.format(val_loss)
    timestamp = datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d_%H:%M:%S')

    # Save model
    model_name = f'fold{i:d}'
    filepath = os.getcwd() + '/models/simple_nn/' + model_name + '__' + timestamp + '__' + val_loss_str + '.h5'
